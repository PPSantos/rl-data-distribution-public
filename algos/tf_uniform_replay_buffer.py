...
class TFUniformReplayBuffer(replay_buffer.ReplayBuffer):
  ...
  def __init__(self,
    ...
    Args:
      ...
      statistics_table_shape: if not None it should specify the shape of the
                table to store the frequency of each (s,a) pair in the replay buffer.
    """
    ...
    self._update_statistics_table = (statistics_table_shape is not None)
    if self._update_statistics_table and batch_size != 1:
      raise ValueError('Batch size must equal 1 when using statistics table.')
    with tf.device(self._device), tf.compat.v1.variable_scope(self._scope):
      ...
      if statistics_table_shape is not None:
        self._statistics_table = tf.Variable(np.zeros(statistics_table_shape))
  ...
  def _add_batch(self, items):
    ...
    with tf.device(self._device), tf.name_scope(self._scope):
      ...
      if self._update_statistics_table:
        ...
        # WARNING: Code below assumes batch_size=1 and first
        # "extra" variable is the env. discrete state.

        if id_ >= self._max_length:
          # Remove overwritten data from the statistics.
          data_to_overwrite = self._data_table.read(write_rows)
          ids_to_overwrite = self._id_table.read(write_rows)
          state = data_to_overwrite[5][0]
          action = data_to_overwrite[1][0]
          self._statistics_table[state,action].assign(self._statistics_table[state,action]-1)

        # Add newly inserted item into the statistics. 
        state = items[5][0]
        action = items[1][0]
        self._statistics_table[state,action].assign(self._statistics_table[state,action]+1)        
      ...
      with tf.name_scope('get_next'):
        ...
        if self._update_statistics_table:
          buffer_info = BufferInfo(ids=data_ids,
                                  ...
                                  statistics=self._statistics_table)
        else:
          buffer_info = BufferInfo(ids=data_ids,
                                  probabilities=probabilities,
                                  statistics=None)
