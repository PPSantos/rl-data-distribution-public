...
args = {
    ...
    'epsilons': [1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0.01, 0.0],
    'num_resamples': 5,
...
if __name__ == '__main__':
    ...
    for eps in args['epsilons']:
        error_ranks_at_max[str(eps)] = []
        errors_at_max[str(eps)] = []
        average_errors[str(eps)] = []
        max_estimation_mistakes[str(eps)] = []
        max_estimation_mistakes_2[str(eps)] = []
    ...
    for (i, Y) in enumerate(Ys):
        ...
        for eps in args['epsilons']:
            ...
            # Calculate epsilon-greedy distribution.
            eps_greedy_dist = [eps/args['nb_of_samples']]*args['nb_of_samples']
            eps_greedy_dist[np.argmax(Y)] += (1-eps)
            ...
            for _ in range(args['num_resamples']):
                ...
                for _ in range(args['resample_size']):
                    idx = np.random.choice(range(len(X)), p=eps_greedy_dist)
                ...
                errors_at_max[str(eps)].append(error_max_idx)
                error_ranks_at_max[str(eps)].append(error_rank)
                average_errors[str(eps)].append(average_error)
                max_estimation_mistakes[str(eps)].append(is_max_estimation_mistake)
                max_estimation_mistakes_2[str(eps)].append(is_max_estimation_mistake_2)
